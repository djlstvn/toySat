...
class StreamBuffer
{
  ...
  { // Makes sure that read position is not out of bound.
    if (pos >= size) {
      pos  = 0;
...
void skipWhitespace(StreamBuffer &in) {
  while ((*in >= 9 && *in <= 13) || *in == 32) // all whitespaces
    ++in;
}
...
void parse_DIMACS_main(StreamBuffer &in, vector<vector<int> > &clauses) {
  while (true) {
    ...
    // c marks the start of a comment line
    // p marks the first line "p cnf [number of literals] [number of clauses]"
